/**
 * Cloudflare Worker entrypoint for CF Worker Insights
 * Serves the Vite React dashboard static assets and proxies telemetry API endpoints
 */

export interface Env {
  CLOUDFLARE_API_TOKEN?: string;
  CLOUDFLARE_ACCOUNT_ID?: string;
  CLOUDFLARE_DEFAULT_WORKER?: string;
  ASSETS?: {
    fetch: (request: Request) => Promise<Response>;
  };
}

const DEFAULT_TOKEN = "cfut_vjkrdUSSo57XcBxW0nK3dSkYJBtwyPHQF1JPgeNZ66cd2bca";
const DEFAULT_ACCOUNT = "59b4d522e7c39647242811ed9584432f";
...
export default {
  async fetch(request: Request, env: Env): Promise<Response> {
    ...
    const token = env.CLOUDFLARE_API_TOKEN || DEFAULT_TOKEN;
    const accountId = env.CLOUDFLARE_ACCOUNT_ID || DEFAULT_ACCOUNT;
    ...
    if (pathname === "/api/download-env") {
      const envContent = [
        `# Cloudflare API Credentials for Vercel / Netlify / Worker Deployment`,
        `CLOUDFLARE_API_TOKEN="${token}"`,
        `CLOUDFLARE_ACCOUNT_ID="${accountId}"`,
        `CLOUDFLARE_DEFAULT_WORKER="${env.CLOUDFLARE_DEFAULT_WORKER || 'speed-cloudflare-com'}"`,
        ``
      ].join("\n");
    ...
    if (pathname === "/api/cloudflare/status") {
      try {
        ...
        const tokenData: any = await tokenVerifyRes.json();

        const accountsRes = await fetch("https://api.cloudflare.com/client/v4/accounts", {
          headers: { Authorization: `Bearer ${token}` },
        });
        const accountsData: any = await accountsRes.json();
        const currentAccount = accountsData.result?.find((a: any) => a.id === accountId) || accountsData.result?.[0];
        ...
        return jsonResponse({
          ...
          accountId: currentAccount?.id || accountId,
          accountName: currentAccount?.name || "Cloudflare Account",
          type: currentAccount?.type || "standard",
        });
      } catch (err: any) {
        return jsonResponse({ connected: false, error: err.message }, 500);
      }
    }

    if (pathname === "/api/cloudflare/workers") {
      try {
        const scriptsRes = await fetch(`https://api.cloudflare.com/client/v4/accounts/${accountId}/workers/scripts`, {
          headers: { Authorization: `Bearer ${token}` },
        });
        const data: any = await scriptsRes.json();

        if (!data.success) {
          return jsonResponse({ error: data.errors?.[0]?.message || "Failed to fetch scripts" }, 400);
        }
    ...
    if (pathname === "/api/cloudflare/metrics") {
      try {
        ...
        const coloCounts: Record<string, { requests: number; cpuUs: number; durationS: number }> = {};
    ...
    if (pathname === "/api/cloudflare/query-sample") {
      const script = url.searchParams.get("scriptName") || "speed-cloudflare-com";
